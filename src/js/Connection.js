/* eslint-disable */
/* eslint-disable no-shadow,no-console,no-alert,func-names, no-param-reassign */
...
class Connection {
  constructor (props) {
    ...
    this.props.protocol = this.props.protocol || globalThis.location.protocol
    this.props.host = this.props.host || (globalThis.location.host && globalThis.location.host.substr(0, globalThis.location.host.indexOf(':')))
    ...
    this.adapterStates = reactive({})
    this.adapterObjects = reactive({})
  ...
  startSocket () {
    ...
    if (typeof globalThis.io === 'undefined') {
      ...
      if (typeof globalThis.registerSocketOnLoad !== 'function') {
        ...
        if (this.scriptLoadCounter < 30) {
          ...
          globalThis.alert('Cannot load socket.io.js!')
        }
      } else {
        // register on load
        globalThis.registerSocketOnLoad(() => this.startSocket())
      }
      return
    } else {
    ...
    this._socket = globalThis.io.connect(
    ...
    this._socket.on('error', err => {
      ...
      if (_err.indexOf('User not authorized') !== -1) {
        ...
        globalThis.alert('Socket Error: ' + err)
      }
    })
    ...
    this._socket.emit('getUserPermissions', (err, acl) => {
      ...
      this._socket.emit('getObject', 'system.config', (err, data) => {
        ...
        if (!this.doNotLoadAllObjects) {
          this.getObjects()
            .then((objects) => {
              Object.assign(this.adapterObjects, objects)
              // console.log(this.adapterObjects)
          ...
          Object.assign(this.adapterObjects, { 'system.config': data })
          // console.log(this.adapterObjects)
  ...
  authenticate () {
    globalThis.location = `${globalThis.location.protocol}//${globalThis.location.host}${globalThis.location.pathname}
    ?login&href=${globalThis.location.search}${globalThis.location.hash}`
  }
  ...
  stateChange (id, state) {
    ...
    for (const task in this.statesSubscribes) {
      if (this.statesSubscribes.hasOwnProperty(task) && this.statesSubscribes[task].reg.test(id)) {
        ...
        Object.assign(this.adapterStates, { [id]: state })
      }
    }
  }
  ...
  _subscribe (isEnable) {
    if (isEnable && !this.subscribed) {
      ...
      this.autoSubscribes.forEach(id => this.subscribeObject(id, (id, obj) => {
        if (obj) {
          Object.assign(this.adapterObjects, { [id]: obj })
        } else {
  ...
  readMetaItems () {
    ...
    return new Promise((resolve, reject) =>
      this._socket.emit('getObjectView', 'system', 'meta', {
        startkey: '',
        endkey: '\u9999'
      }, (err, objs) =>
