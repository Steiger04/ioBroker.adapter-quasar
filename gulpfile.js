...
const replace = 'adapter-quasar'

const changeFileName = (cb) => {
  fs.rename('src/assets/' + regex + '.png', 'src/assets/' + (arg.name ? arg.name : replace) + '.png', (err) => {
...
const changeAdapterName = (cb) => {
  gulp.src(['README.md', 'package.json', 'io-package.json', 'src/boot/init.js', 'src/components/*.vue',
    'src/layouts/MainLayout.vue'], { base: './' })
    .pipe(replaceRegex({ regex: regex, replace: arg.name ? arg.name : replace }))
    .pipe(gulp.dest('.'))
